pro smart_paper2_20110330, res1tore=res1tore, res2tore=res2tore, res3store=res3store, res4store=res4store, $
...
save,nlincent,nlinlo,nlinhi,nlinlogt22,nlinhigt22,nlinlogt5d22,nlinhigt5d22,slincent,slinlo,slinhi,slinlogt22,slinhigt22,slinlogt5d22,slinhigt5d22, $
...
;Plot centroid lines
utplot,xarr-mintimseries,nfluxcent,mintimseries,yran=[-60,60],/xsty,pos=[.12,.525,.95,.95],xtickname=strarr(10)+' ',xtit='',/nodata,ytit='Latitude',thick=5
...
vline,xarr[wmiss]-mintimseries,color=!white,thick=15
...
plot_image,fluxstack,/noerase,pos=[.12,.1,.95,.525],color=255,/xsty,/nosq

utplot,xarr-mintimseries,nfluxcent,mintimseries,yran=[-60,60],/xsty,thick=3,pos=[.12,.1,.95,.525],/nodata,/noerase,ytit='Latitude',xtit='Year (Begins 1 June 1996)'
vline,xarr[wmiss]-mintimseries,color=!white,thick=15
hline,0,color=!gray,thick=10
...
utplot,tfluxyrbin-reftim,fluxmeanyrbin*1d16,reftim,ps=10,position=[.12,.67,.97,.83],/noerase,ytitle='<'+textoidl('\Phi')+'> [Mx]',/xsty,xtit='Year (Begins 1 January 1996)'
vline,[trise,tmax,tdecay,tend]-reftim,thick=3,color=!gray
...
oplot,xfitareamax, fitareamax,color=!red,ps=-1
...
oplot,xfitfluxmax, fitfluxmax,color=!red,ps=-1
...
for i=0,nbinscrop-1 do begin
	if nbmask[i] then begin
		nbsignedunipole[i]=mean(magrebinhigh[i,nfluxmmcrop[1,i]+90.:nfluxmmcrop[1,i]+90.+10.])
		magrebinhighmask[i,nfluxmmcrop[1,i]+90.:nfluxmmcrop[1,i]+90.+10.]=1
	endif
	if sbmask[i] then begin
		sbsignedunipole[i]=mean(magrebinhigh[i,sfluxmmcrop[0,i]+30.-10.:sfluxmmcrop[0,i]+30.])
		magrebinhighmask[i,sfluxmmcrop[0,i]+30.-10.:sfluxmmcrop[0,i]+30.]=1
	endif
endfor
...
xyouts,.7,.9,'<B'+textoidl('_{SIGNED}')+'> [G]',/norm,color=255
...
xyouts,.7,.47,'Detection '+textoidl('\Phi_{NET}')+' [MX]',/norm,color=0
...
;polar plot
setcolors,/sys
utplot,timrebin-mintim,npole6065,mintim,yran=[-10,10], pos=[.12,.525,.95,.95],ps=10,xran=minmax(timrebin)-mintim,/xsty,ytit='Polar Fields [G]',xtit='',ytickname=strarr(10)+' '
